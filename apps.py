...
st.set_page_config(page_title="Project Work",
...
emg_data = pd.read_csv('train_data.csv')
...
# Display the headers
st.markdown(
  "<h1 style='text-align: center; color: #14425A;'>Silent Speech Interface of EMG Signals</h1>",
  unsafe_allow_html=True)
st.markdown(
  "<h2 style='text-align: center; color: #14425A;'>Govt SKSJTI, ECE DEPT</h2>",
  unsafe_allow_html=True)
st.markdown(
  "<h3 style='text-align: center; color: #14425A;'>Project Team 09: Ananad, Kiran Kumar S, Lakshmana and Mahesh S</h3>",
  unsafe_allow_html=True)
st.markdown(
  "<h4 style='text-align: center; color: #14425A;'>Guided by Dr. N. Sathisha</h4>",
  unsafe_allow_html=True)
...
st.sidebar.write(
  "3. Wait for the prediction results and test metrics to appear.")
st.sidebar.write(
  "4. Check the highlighted rows in the prediction results table for incorrect predictions."
)
st.sidebar.write(
  "5. View the train metrics and prediction results on the training dataset.")

# Get the test data from user
st.sidebar.subheader("Upload Test Data")
test_data = st.sidebar.file_uploader("Choose a CSV file containing test data",
                                     type="csv")
...
if test_data is not None:
  ...
  y_pred_new, accuracy_new, precision_new, recall_new, f1_new, label_accuracies_new = predict_and_evaluate(
    loaded_model, X_new, y_new)
  ...
  results_df = results_df.style.applymap(
    lambda x: f'background-color: {highlight_color}',
    subset=pd.IndexSlice[incorrect_pred_mask, ['Predicted Labels']])
  ...
  st.sidebar.warning(
    "Please upload test data file to see prediction results and test metrics.")

# Display train accuracy
_, accuracy_train, precision_train, recall_train, f1_train, label_accuracies_train = predict_and_evaluate(
  loaded_model, X, y)
st.subheader("Train Metrics")
st.write("- Accuracy on training dataset: {:.2f}%".format(accuracy_train *
                                                          100))
...
train_results_df = pd.DataFrame({
  'Actual Labels': y,
  'Predicted Labels': loaded_model.predict(X)
})
st.write(train_results_df)

# Display main screen image
main_image = "mainscreen.jpeg"
